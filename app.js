const EMBEDDED_QUESTIONS = [
  ...
  },
  {
    ...
    "options": [
      ...
      "B) Предложить отсканировать QR с кассовой зоны (бот): Старт → Регистрация/авторизация → номер через +7",
      "C) Попросить паспорт и завести карту без бота и QR",
      "D) Отправить клиента в офис за пластиковым носителем"
    ],
    "answer": "b",
    "explain": "QR с POS на кассе → бот (Старт, регистрация, номер через +7). После регистрации сканируем QR клиента и применяем карту к этой покупке."
  },
  {
    "id": 7,
    "type": "mc",
  ...
  },
  {
    ...
    "minKeywords": 4,
    "sample": "Предложить QR с POS на кассе → бот MAX (или Telegram / ЛК на gvozditut.ru): Старт → Регистрация/авторизация → номер через +7. После регистрации клиент открывает QR, кассир сканирует его — карта применяется к текущей покупке, бонусы начислятся в течение 3 дней."
  },
  {
    "id": 20,
    "type": "open",
...
const TOPIC_TIPS = {
  ...
  Регистрация:
    "Бот MAX / ЛК gvozditut.ru / QR на кассе: номер +7. Затем сканируем QR клиента.",
...
function grade() {
  ...
  if (correct >= Math.ceil(total * 0.86)) {
    ...
    verdict = "Хорошо — база есть, разберите ошибки ниже: скрипт, QR и перенос по заявлению.";
