const EMBEDDED_QUESTIONS = [
  ...
  },
  {
    ...
    "options": [
      ...
      "B) Предложить отсканировать QR с кассовой зоны (бот): Старт → Регистрация/авторизация → номер через +7 → код из СМС. При необходимости — кнопка «Регистрация клиента в СУЛ» на кассе и код из СМС",
      "C) Попросить паспорт и завести карту без СМС",
      "D) Отправить клиента в офис за пластиковым носителем"
    ],
    "answer": "b",
    "explain": "Обучение кассира 2.11: QR с POS на кассе → бот (Старт, регистрация, +7, СМС) либо кнопка «Регистрация клиента в СУЛ». Без кода из СМС карта не регистрируется. После регистрации сканируем QR клиента и применяем карту к этой покупке."
  },
  {
    "id": 7,
    "type": "mc",
